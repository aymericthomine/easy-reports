            ...
            <div>
                ...
                <div class="modal-body">
                    ...
                    <form wire:submit.prevent="closeModal()" style="padding-left: 30px; padding-right: 30px; padding-bottom: 30px;" class="flex flex-col justify-center">
                        @method( isset($company_id) ? 'PUT' : 'POST' )
                        @csrf
                        ...
                        <div class="form-group flex flex-col">
                            <div class="mt-4">
                                <x-jet-label value="{{ __('Siret') }}" />
                                <x-jet-input type="siret" class="block mt-1 w-full" name="siret" for="siret" id="siret" placeholder="Siret"
                                             value="{{ old('siret') }}"
                                             wire:model.lazy="siret"
                                             action="{{ $modalAction }}"
                                />
                                <x-jet-input-error for="siret" class="mt-2" />
                            </div>
                        </div>

                        <div class="form-group flex flex-col">
                            <div class="mt-4">
                                <x-jet-label value="{{ __('Iban') }}" />
                                <x-jet-input type="iban" class="block mt-1 w-full" name="iban" for="iban" id="iban" placeholder="Iban"
                                             value="{{ old('iban') }}"
                                             wire:model.lazy="iban"
                                             action="{{ $modalAction }}"
                                />
                                <x-jet-input-error for="siret" class="mt-2" />
                            </div>
